...
import {isStr, simplifyFileName, isFn, trimSpace} from '@ar/util'
    ...
    <Fragment>
      {
        data.map((ele, i) => {
          ...
          return (
            <a href={noHref ? 'javascript:void(0);' : ele} key={-i} rel="noopener noreferrer"><img src={ele} alt='pic'
                                                                                                   width='86px'
                                                                                                   style={{...extendStyle}}/></a>)
        })
      }
...
export const MainPic = (props) => {
  ...
  if (url && isPrivate) {
    return <a href={url} rel="noopener noreferrer" target="_blank"><img src={`${url}`} alt='pic' height='100px'
                                                                        width='86px' style={{...style}}/></a>
  }

  if (url) {
    return <a href={url} rel="noopener noreferrer" target="_blank"><img
      src={`${url}?x-oss-process=image/resize,m_fixed,h_${h},w_${w}`} alt='pic' height='100px' width='86px'
      style={{...style}}/></a>
  }
  return <a href='javascript:void(0);' rel="noopener noreferrer" target="_blank"><img
    src='https://asman-img.oss-cn-hangzhou.aliyuncs.com/noPic_0e7bffac7958f603a8b37fe3cda07499.png' alt='pic'
    height='100px' width='86px' style={{...style}}/></a>
}
...
export const SomeRed = (props) => {
  ...
  const trimkey = trimSpace(keyWord);
...
export const LRInfo = (props) => {
  ...
  return (<div className='df'>
    <div className='g0' {...titleProps}>{title || ' -- '}&nbsp;{hasColon ? ':' : null} &nbsp;</div>
