...
function arRoundNum(num, roundType) {
...
function fixedZero(num, numCount, roundType) {
    ...
    var newValue = arRoundNum(num, roundType);
 ...
 * */
function getPlainNode(nodes, parentPath) {
...
function getRelation(str1, str2) {
    ...
    if (arr1.every(function (item, index) { return item === arr2[index]; })) {
        return 2;
    }
    return 3;
}
...
function getRenderArr(routes) {
    ...
    var _loop_1 = function (i) {
        ...
        // 是否包含
        var isAdd = renderArr.every(function (item) { return getRelation(item, routes[i]) === 3; });
        if (isAdd) {
...
function getRoutes(path, routerData) {
    ...
    return renderArr.map(function (item) {
        var exact = !routes.some(function (route) { return route !== item && getRelation(route, item) === 1; });
        var keyName = "" + path + item;
        return __assign(__assign({ exact: exact }, (routerData[keyName])), { key: "" + path + item, path: "" + path + item });
    });
}
...
function formatWan(val, roundType) {
    if (roundType === void 0) { roundType = UtilRoundType.Round; }
    var resNum = arRoundNum(val, roundType);
    if (!resNum || Number.isNaN(resNum))
        return '';
    if (resNum < 10000) {
        return (jsxRuntime.jsx("span", { children: resNum }, void 0));
    }
    var result = Math.floor(resNum / 10000);
    return (jsxRuntime.jsxs("span", { children: [result, jsxRuntime.jsx("span", __assign({ style: {
                    position: 'relative',
                    top: -2,
                    fontSize: 14,
                    fontStyle: 'normal',
...
export { UtilRoundType, arRoundNum, fixedZero, formatWan, getPageQuery, getPlainNode, getQueryPath, getRelation, getRenderArr, getRoutes, isUrl };
